...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant==None:
                ...
                if self.ant.container and insect.container:
                    assert self.ant is None, 'Two ants in {0}'.format(self)
                elif self.ant.container and self.ant.ant is None:
                    self.ant.contain_ant(insect)
                elif insect.container and insect.ant is None:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            "*** YOUR CODE HERE ***"
            if type(insect)==QueenAnt and insect.count==1:
                return self.ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        loc = self.place
        i = 0
        range1,range2=self.min_range,self.max_range
        while loc.entrance != None or loc != hive:
            if len(loc.bees)> 0 and i >= range1 and i <= range2:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        "*** YOUR CODE HERE ***"
        for bees in colony.bees[:]:
            if self.place==bees.place:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting==0:
            for bees in colony.bees[:]:
                if self.place==bees.place:
                    bee_list.append(bees)
            bee=random_or_none(bee_list)
            if bee==None:
                return self.digesting
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.count>1:
            ...
            action=ThrowerAnt.action(self,colony)
            loc,loc1=self.place,self.place
            while loc.entrance!=None or loc1.exit!=None:
                for ant in colony.ants[:]:
                    if ant.place==loc or ant.place==loc1:
                        if ant not in self.ants and ant.name!="Queen":
                            if ant.name=="Bodyguard" and ant.ant.name!="Queen":
                                ant.ant.damage=ant.ant.damage*2
                                self.ants.append(ant.ant)
                            ant.damage=ant.damage*2
                            self.ants.append(ant)
                if loc.entrance==None:
                    loc,loc1=loc,loc1.exit
                elif loc1.exit==None:
                    loc,loc1=loc.entrance,loc1
                else:
                    loc,loc1=loc.entrance,loc1.exit
            return action
